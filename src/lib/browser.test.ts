...
import { copyText, randomId, useAutoClearTimeout } from "./browser";
...
describe("useAutoClearTimeout", () => {
  ...
  it("fires the scheduled callback after the delay", () => {
    vi.useFakeTimers();
    const { result } = renderHook(() => useAutoClearTimeout());
    const fn = vi.fn();
  ...
  it("replaces a pending timer rather than stacking it", () => {
    vi.useFakeTimers();
    const { result } = renderHook(() => useAutoClearTimeout());
    const first = vi.fn();
    const second = vi.fn();
  ...
  it("cancels a pending timer on unmount (#989)", () => {
    vi.useFakeTimers();
    const { result, unmount } = renderHook(() => useAutoClearTimeout());
    const fn = vi.fn();
