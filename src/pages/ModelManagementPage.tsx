...
function ModelDetailPanel({ modelId }: { modelId: string }) {
  ...
  const { data: detail, error, loading } = useAsyncResource(
    (signal) => apiFetch<ModelDetail>(`/v1/models/${encodeURIComponent(modelId)}`, { signal }),
    { deps: [modelId] },
  );

  return (
    <div className="mt-1 text-xs text-gray-600 dark:text-gray-400">
      {(error ?? (loading ? "Loading…" : "")) && <p>{error ?? "Loading…"}</p>}
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="flex items-center gap-2 text-base font-semibold text-gray-900 dark:text-gray-100">
            <Icon name="models" size={17} className="shrink-0 text-indigo-500 dark:text-indigo-400" />
            {model.name}
  ...
  return (
    <section className={`mb-6 ${cardClass}`}>
      <h2 className="mb-1 flex items-center gap-2 text-base font-semibold text-gray-900 dark:text-gray-100">
        <Icon name="download" size={16} className="text-indigo-500 dark:text-indigo-400" />
        Pull a model
      </h2>
      ...
      {pulling && (
        <StatusLine>
          <Icon name="reset" size={13} className="mr-1 inline animate-spin align-[-1px]" /> Pulling —{" "}
          {formatElapsed(elapsed)} elapsed. This blocks until the download
          finishes (can take several minutes for large models) — don't
          navigate away.
        </StatusLine>
      )}
      {!pulling && <StatusLine isError={isError}>{status}</StatusLine>}
    </section>
  );
}
