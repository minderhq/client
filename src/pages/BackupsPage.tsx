    ...
    <div className="mb-2 rounded-lg border border-gray-200 p-3 text-sm dark:border-gray-700">
      <div className="flex flex-wrap items-center gap-2">
        <span className="flex items-center gap-1.5 font-medium text-gray-900 dark:text-gray-100">
          <Icon
            name={job.action === "backup" ? "backups" : "undo"}
            size={14}
            className="text-gray-400"
          />
          {job.action === "backup" ? "Backup" : "Restore"}
        </span>
        {job.archive && (
...
export function BackupsPage() {
  ...
  async function handleTriggerBackup() {
    if (triggering) return; // already in flight -- ignore a double-click/tap
    setTriggering(true);
  ...
  return (
    <>
      <PageHeader
        icon="backups"
        title="Backups"
        subtitle="Backup and restore run as a job queue, not directly from this page — a host-side process picks up each request and does the real work (see Recent Jobs below for progress). Admin-only: archive existence and timestamps are treated as sensitive operational detail."
      />
